...
use_hard_boundary = False  # %%% Modify this flag to switch methods %%%
...
class PINN(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Linear(1, 20),
            nn.Tanh(),
            # %%% (Optional) Modify architecture if desired %%%
            nn.Linear(20, 20),
            nn.Tanh(),
            nn.Linear(20, 1)
        )
    ...
    def forward(self, x):
        ...
        if use_hard_boundary:
            # %%% Fill in: Construct the trial solution that automatically satisfies the boundary conditions.
            # Hint: Use the ansatz u(x) = (2*x/np.pi) + x*(x1 - x)*out.
...
def ode_residual(model, x):
    ...
    u = model(x)
    # %%% Fill in: Compute the first derivative u_x using torch.autograd.grad %%%
    u_x = torch.autograd.grad(u, x, grad_outputs=torch.ones_like(u), create_graph=True)[0]
    # %%% Fill in: Compute the second derivative u_xx using torch.autograd.grad %%%
...
for epoch in range(n_epochs):
    ...
    if not use_hard_boundary:
        ...
        # %%% Fill in: Apply boundary condition at x = 0: u(0)=0 %%%
        x_left = torch.tensor([[x0]], requires_grad=True)
        u_left = model(x_left)
        loss_bc_left = (u_left - 0.0)**2
        
        # %%% Fill in: Apply boundary condition at x = x1: u(x1)=1 %%%
        ...
        # Hard method: the trial solution already enforces boundaries, so only PDE loss is needed.
        loss = loss_pde
